#!/usr/bin/env node
/*! Generated by tools/build-scripts.js — DO NOT EDIT directly. Source lives in scripts/_src/. Rebuild: `npm run build:scripts`. */
"use strict";
...
// plugins/axiom/skills/axiom/scripts/_src/validate-coded.js
var { readFileSync, existsSync } = require("node:fs");
var { resolve } = require("node:path");
var acorn = require_acorn();
var walk = require_walk();
var ALLOWLIST_PATH = resolve(__dirname, "..", "references", "axiom-api-method-allowlist.json");
var BLOCKLIST_PATH = resolve(__dirname, "..", "references", "axiom-api-method-blocklist.json");
var ALLOWLIST = loadMethodList(ALLOWLIST_PATH);
var BLOCKLIST = loadMethodList(BLOCKLIST_PATH);
...
function validateCodedScript(source) {
  ...
  const axiomInstances = /* @__PURE__ */ new Set();
  ...
  walk.ancestor(ast, {
    ...
    VariableDeclarator(node) {
      if (node.id && node.id.type === "Identifier" && node.init && node.init.type === "NewExpression" && node.init.callee && node.init.callee.type === "Identifier" && node.init.callee.name === "AxiomApi") {
  ...
  function checkCall(call, ancestors) {
    ...
    if (method === "browserOpen") {
      const enclosingFn = ancestors.slice().reverse().find(
