---
title: Get a list of links from a URL step
description: Scrape links from a URL. Return results via a token for use in your automation.
category: Scrape
...
Use this step to scrape links from listing-style pages. Then pass the links into a ['Get data from a webpage'](/docs/no-code-tool/reference/steps/Get-data-from-website) step to scrape data. Alternatively, write the links to a  ['Google sheet'](/docs/no-code-tool/reference/steps/read-data-from-a-google-sheet-step) and create a batch scraper for large-scale scraping. We call this design pattern 'Batch scraping' Learn more about ['batching here'](/docs/batching-bot-runs) or try this [template](/guides/batching). You can also find a ['Link scraper template here.'](/guides/scrape-links-from-a-website)
