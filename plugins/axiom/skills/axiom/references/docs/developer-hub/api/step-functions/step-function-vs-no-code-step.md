...
| [**Get data from bot's current page**](/docs/no-code-tool/reference/steps/scrape) | [`axiom.scrape(url, selector, pager, max_results, settings)`](/docs/developer-hub/api/step-functions/scrape) | `axiom.scrape` navigates to `url` and extracts in one call — `url` is **required**, do not pass `null`. Browser session/cookies persist across the internal navigation, so this works after a login flow too. |
