...
| 2 | `build_no_code` | Emit a no-code AutomationTemplate JSON, validate, then offer to save it to the user's Axiom account directly (falls back to manual import if declined / fails). | `scripts/validate-no-code.js`, `scripts/save-automation.js` (POST `/api/v4/automation`) |
